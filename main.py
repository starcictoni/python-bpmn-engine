...
class ProcessDefinition:
    ...
    def get_process_definition_by_id(self, request):
        process_definition_id = request.match_info.get('id')
        if utils.is_valid_type_id(process_definition_id) == False:
            return web.json_response(status=400, data={})
        try: 
            response = db_connector.get_process_definition_by_id(process_definition_id)
            data = json.dumps(response, sort_keys=True, default=str)
        except Exception as error:
            traceback.print_stack(error)
            return web.json_response(status=400, data={})
    ...
    def get_process_definition_by_key(self, request):
        process_definition_key = request.match_info.get('key')
        if utils.is_valid_uuid(process_definition_key) == False:
            return web.json_response(status=400, data={})
        try: 
            response = db_connector.get_process_definition_by_key(process_definition_key)
            data = json.dumps(response, sort_keys=True, default=str)
        except Exception as error:
            traceback.print_stack(error)
            return web.json_response(status=400, data={})
